...
<?php
	...
	if(isset($_GET['delete'])){
		...
		// seleciona a imagem
		$seleciona = "SELECT * from tb_galeria WHERE id= :id_delete";
		try{
			...
			if($contar>0){
				...
				$arquivo = "../upload/galeria/" .$fotoDeleta;
				unlink($arquivo);
				
				
				// exclui o registo
				$seleciona = "DELETE from tb_galeria WHERE id=:id_delete";
				try{
					$result = $conexao->prepare($seleciona);
					$result->bindParam('id_delete',$id_delete, PDO::PARAM_INT);				
					$result->execute();
...
if(!is_numeric($pg)){
	...
	echo '<script language= "JavaScript">
					location.href="home.php?acao=ver-galeria";
		</script>';
}
...
if(isset($_POST['palavra-busca'])){
	...
	$select = "SELECT * from tb_galeria WHERE titulo LIKE '%$busca%' ORDER BY id DESC LIMIT $inicio, $quantidade";	
}else{
	$select = "SELECT * from tb_galeria ORDER BY id DESC LIMIT $inicio, $quantidade";
}
...
?>           
                  <tr>
                    ...
                    <td><img src="../upload/galeria/<?php echo $mostra->imagem;?>" width="50"/></td>
...
<?php
				...
				echo '<div class="alert alert-danger">
                      <button type="button" class="close" data-dismiss="alert">×</button>
                      <strong>Aviso!</strong> Não há fotos cadastradas em nosso banco de dados ou a página não existe.
                </div>';
			}
...
if(isset($_POST['palavra-busca'])){
	$busca = addslashes($_POST['palavra-busca']);
	$sql = "SELECT * from tb_galeria WHERE titulo LIKE '%$busca%'";	
}else{
...
<div class="paginas">
	...
	<a href="home.php?acao=ver-galeria&pg=1">Primeira Página</a>
    ...
    <a href="home.php?acao=ver-galeria&pg=<?php echo $i;?>"  class="ativo<?php echo $i;?>"><?php echo $i;?></a>
     
         
<?php  }} ?>
    
    
    <a href="home.php?acao=ver-galeria&pg=<?php echo $pg;?>" class="ativo<?php echo $i;?>"><?php echo $pg;?></a>
...
<a href="home.php?acao=ver-galeria&pg=<?php echo $paginas;?>">Última página</a>		
